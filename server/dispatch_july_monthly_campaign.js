...
async function runJulyCampaign() {
    console.log('=== DISPARANDO CAMPAÑA MENSUAL JULIO 2026 (MAX 180 CORREOS) ===\n');
    ...
    // Sort by newest activity and take top 180
    const allJulyUsers = Array.from(userMap.values()).sort((a, b) => b.newestDate - a.newestDate);
    const targetUsers = allJulyUsers.slice(0, 180);

    console.log(`📊 Total candidatos encontrados en Julio: ${allJulyUsers.length}`);
    console.log(`🎯 Seleccionados los ${targetUsers.length} más recientes para el lote de hoy.\n`);
    ...
    for (let i = 0; i < targetUsers.length; i++) {
        ...
        if (u.masterLic) {
            ...
            // Create fresh Easy Master trial
        ...
        await sleep(350); // Small pause between emails
    }

    console.log(`\n========================================`);
    console.log(`🎉 CAMPAÑA DE JULIO FINALIZADA`);
