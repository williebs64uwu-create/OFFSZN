...
async function runJuneBatch() {
    console.log('=== DISPARANDO LOTE DE JUNIO 2026 (MAX 63 CORREOS) ===\n');
    ...
    // Exclude licenses whose expires_at is between 36h and 80h from now (all licenses extended today)
    const alreadyNotifiedUserEmails = new Set();
    licenses.forEach(l => {
        if (!l.expires_at) return;
        const diffHours = (new Date(l.expires_at) - now) / (1000 * 60 * 60);
        if (diffHours > 36 && diffHours <= 80) {
    ...
    for (const l of licenses) {
        ...
        // June 2026 filter (or older)
        const junEnd = new Date('2026-07-01T00:00:00.000Z');
        if (created >= junEnd) continue; // Exclude July and August
    ...
    const juneUsers = Array.from(userMap.values()).sort((a, b) => b.newestDate - a.newestDate);
    const targetUsers = juneUsers.slice(0, 63);

    console.log(`📊 Total usuarios únicos disponibles de Junio (sin repetir): ${juneUsers.length}`);
    console.log(`🎯 Seleccionados exactamente los ${targetUsers.length} más recientes para agotar el cupo de hoy sin exceder Brevo.\n`);
    ...
    console.log(`\n========================================`);
    console.log(`🎉 LOTE DE JUNIO FINALIZADO`);
