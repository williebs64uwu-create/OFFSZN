...
function extractKeyFromSignedUrl(url) {
    ...
    if (!url.includes('X-Amz-Signature') && !url.includes('token=')) return url;
...
async function cleanKoimattoruDBUrls() {
    console.log('Fetching koimattoru user...');
    ...
    console.log('Fetching products to clean...');
    ...
    if (products && products.length > 0) {
        ...
        for (const p of products) {
            ...
            console.log(`[${p.id}] CURRENT IMG:`, p.image_url?.substring(0, 50) + '...');
        ...
        if (needsCleaning === 0) {
            console.log('No products had X-Amz- in their URLs.');
        }
    } else {
