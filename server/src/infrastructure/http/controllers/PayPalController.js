            ...
            (async () => {
                try {
                    ...
                    for (const item of cartItems) {
                        ...
                        // Detect Affiliate for Plugin Referral Sales (Allan, Patrick, Ximo, etc.)
                        const incomingAffiliate = (req.body.affiliate || '').toString().trim().toLowerCase();
                        let affiliateInfo = null;

                        if (isPlugin && incomingAffiliate) {
                            // Commission rules: $10 sale -> $2 commission | $15+ sale -> $5 commission
                            let commissionAmount = 0;
                            if (displayTotal >= 15) {
                                commissionAmount = 5.00;
                            } else if (displayTotal >= 10) {
                                commissionAmount = 2.00;
                            } else {
                                commissionAmount = Math.round(displayTotal * 0.20 * 100) / 100;
                            }

                            const netForWillie = Math.max(0, displayTotal - commissionAmount);
                            affiliateInfo = {
