...
async function generateLicensePdf() {
  const purchaseData = {
    productName: 'trust who? - DRUMLESS',
    producerName: 'prodbyeme',
    amount: 30.00,
    buyerName: 'maiaverse69',
    buyerEmail: 'maiaverse69@gmail.com',
    purchaseDate: '2026-08-14T19:17:43.000Z',
    orderId: 1966,
    productType: 'beat',
  ...
  const filesDelivered = 'high-quality WAV, MP3, and individual trackout stem files';
...
IN WITNESS WHEREOF, the parties have executed this Agreement as of the date of purchase.`;
  ...
  const outputPath = path.resolve('..', 'OFFSZN_License_trust_who_Order1966.pdf');
  fs.writeFileSync(outputPath, Buffer.from(pdfBytes));
  console.log('PDF saved to:', outputPath);
}

generateLicensePdf().catch(console.error);
