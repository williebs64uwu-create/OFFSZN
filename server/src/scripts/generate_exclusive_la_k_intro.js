...
async function generateExclusiveLicensePdf() {
  ...
  // Authenticity Code
  const verificationHash = Buffer.from(`${orderId}:${buyerName}:${productName}:${amount}USD:${buyerEmail}`).toString('base64').substring(0, 18).toUpperCase();
  ...
  pdfDoc.setKeywords(['OFFSZN', 'Exclusive License', 'Willie Inspired', 'La K', 'intro', '35 USD', buyerEmail, verificationHash]);
  ...
  // Divider Line
  page1.drawLine({
    start: { x: 45, y: height - 74 },
    end: { x: width - 45, y: height - 74 },
    thickness: 1,
  ...
  let currentDetailY = cardY + cardHeight - 44;
  details.forEach(([label, val], idx) => {
    ...
    page1.drawText(val, {
      ...
      size: 8.2,
      font: boldFont,
      color: idx === 4 ? rgb(0.3, 0.9, 0.6) : (idx === 0 || idx === 2 || idx === 3 ? rgb(1, 1, 1) : rgb(0.9, 0.9, 0.95))
    });
    currentDetailY -= 17.5;
  });

  // Rights Matrix Grid (The 6 "TODO ILIMITADO" pillars)
  const matrixY = height - 520;
  ...
  matrixItems.forEach((item, index) => {
    ...
    const boxY = matrixY - 40 - row * (colHeight + 10);
  ...
  page2.drawText(`(Accepted via License Settlement - $35.00 USD)`, {
    x: sig2X + 10,
    y: sigBoxY + 36,
    size: 6.2,
    font: obliqueFont,
  ...
  page2.drawText(`Email: ${buyerEmail} • #${orderId}`, {
    x: sig2X + 10,
    y: sigBoxY + 23,
    size: 6.5,
    font,
  ...
  const rootOutputPath = path.resolve(process.cwd(), '..', 'OFFSZN_Licencia_Exclusiva_intro_La_K.pdf');
  fs.writeFileSync(rootOutputPath, Buffer.from(pdfBytes));
  
  // Also copy to current server folder for reference
  const serverOutputPath = path.resolve(process.cwd(), 'OFFSZN_Licencia_Exclusiva_intro_La_K.pdf');
  fs.writeFileSync(serverOutputPath, Buffer.from(pdfBytes));
  
  console.log('✅ Final Master Exclusive License PDF generated at:', rootOutputPath);
