...
IN WITNESS WHEREOF, the parties have executed this Agreement as of the date of purchase.`;
  ...
  const file1 = path.join(rootDir, 'OFFSZN_License_intro_Order2026.pdf');
  const file2 = path.join(rootDir, 'OFFSZN_Licencia_Exclusiva_intro_La_K.pdf');

  fs.writeFileSync(file1, Buffer.from(pdfBytes));
  fs.writeFileSync(file2, Buffer.from(pdfBytes));

  console.log('✅ Generated official OFFSZN PDF at:', file1);
  console.log('✅ Also saved at:', file2);
}
