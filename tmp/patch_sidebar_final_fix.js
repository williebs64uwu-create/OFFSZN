...
// We use inline styles for the skeleton blocks to ensure they work even if the CSS hasn't loaded fully
...
// 2. CSS with Animation - Ensuring the animation is present
const profileCSS = `
...
// 3. Updated JS with Skeleton Hiding Logic
const profileJS = `
...
files.forEach(file => {
    ...
    // --- CLEANUP DUPLICATES AND OLD LOGIC ---
    // Remove all profile card blocks
    content = content.replace(/<!-- Profile Card -->[\s\S]*?<\/div>(\s*<!-- Profile Card -->[\s\S]*?<\/div>)*/g, '');
    ...
    // 2. Inject Profile Card at the top of sidebar-stub
    const sidebarStubPattern = /<div class="sidebar-stub">/;
    if (sidebarStubPattern.test(content)) {
        content = content.replace(sidebarStubPattern, '<div class="sidebar-stub">' + profileCardHTML);
    }
    ...
    console.log('Fixed & Upgraded Sidebar:', path.basename(file));
});

console.log('\\nDone! Sidebar fully fixed and upgraded with skeleton loaders in', files.length, 'files.');
