...
const YouTubeUploader = (function () {
    ...
    async function renderVideoWasm(coverBlob, audioBlob, onStatusUpdate) {
        const status = (msg) => {
            if (typeof onStatusUpdate === 'function') onStatusUpdate(msg);
        };

        status('⏳ Cargando motor FFmpeg.wasm...');
        ...
        if (!window._ffmpegWasmLoaded) {
            await new Promise((resolve, reject) => {
                ...
                s.onload = resolve;
                s.onerror = () => reject(new Error('No se pudo cargar FFmpeg.wasm desde CDN'));
        ...
        ffmpeg.setLogger(({ type, message }) => {
            if (type === 'fferr' && message.includes('time=')) {
                ...
                if (match) {
                    status(`🎬 Renderizando video en tu PC... (${match[1]})`);
                }
            }
        });

        status('⏳ Inicializando motor en navegador...');
        await ffmpeg.load();

        status('📁 Preparando portada y audio...');
        ffmpeg.FS('writeFile', 'cover.jpg', await fetchFile(coverBlob));

        const isWav = audioBlob?.type?.includes('wav');
        const audioFile = isWav ? 'audio.wav' : 'audio.mp3';
        ffmpeg.FS('writeFile', audioFile, await fetchFile(audioBlob));

        status('🎬 Procesando video MP4 en tu navegador...');
        ...
        const videoBlob = new Blob([outData.buffer], { type: 'video/mp4' });
    ...
    async function renderPreviewUI(containerId, data) {
        ...
        if (btnRender) {
            btnRender.onclick = async () => {
                ...
                try {
                    await renderVideoWasm(data.coverBlob, data.audioBlob, (msg) => {
