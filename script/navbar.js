    ...
    searchInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
            ...
            if (NavbarState.search.selectedIndex >= 0 && items[NavbarState.search.selectedIndex]) {
                ...
                if (val) window.saveToHistory(val);

                console.log("Searching for:", val);

                // Perform search and auto-redirect if exact match
...
async function performSearch(query, category, autoRedirectExact = false) {
    ...
    try {
        ...
        if (typeof window.supabaseClient !== 'undefined') {
            ...
            // PRO FUZZY SEARCH: Use the new RPC function for better resilience (typos, spaces)
            ...
            // USERS FUZZY SEARCH: Run if 'Todo' OR explicitly 'Productores'
            ...
            // DEFENSIVE: Ensure RPC responses are valid arrays (400 errors return strings/nulls)

            if (pRes.error || !Array.isArray(pRes.data)) pRes.data = [];

            if (uRes.error || !Array.isArray(uRes.data)) uRes.data = [];



            // --- FALLBACK: IF NO RESULTS MATCH, FETCH TOP 3 ---

            if (pRes.data.length === 0 && uRes.data.length === 0) {

                const { data: popular } = await window.supabaseClient

                    .from('products')

                    .select('id, name, price_basic, image_url, product_type, producer_id, public_slug')
            ...
            if (pRes.data.length > 0) {
                ...
                realResults = [...realResults, ...pRes.data.map(p => {
                    ...
                    return {
                        ...
                        type: p.product_type === 'beat' ? 'beat' : 'kit', // For icon logic

                        product_type: p.product_type,                   // Needed for SEO link

                        name: p.name,                                   // Needed for SEO link

                        public_slug: p.public_slug,                     // Needed for SEO link
            ...
            if (uRes.data.length > 0 && realResults.length < 3) {
        ...
        if (autoRedirectExact && realResults.length > 0) {

            // Find EXACT match (case-insensitive)

            const exactMatch = realResults.find(r =>

                r.title.toLowerCase() === query.toLowerCase().trim() ||

                (r.name && r.name.toLowerCase() === query.toLowerCase().trim())
...
function renderActualResults(results) {
    ...
    results.forEach(item => {
        ...
        // CURRENCY LOGIC - Use global CurrencyManager

        let displayPrice = item.price;

        if (window.CurrencyManager && item.price && item.price !== 'Free') {
        ...
        if (item.img) {
            ...
            imgHtml = `<img data-r2-src="${item.img}" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" style="width:36px; height:36px; border-radius:6px; object-fit:cover; background:#1a1a1a;" alt="cover">`;
        ...
        // Store the item data safely as base64 to avoid any HTML attribute escaping issues
        ...
        const itemDataObj = {
            ...
            subtitle: item.producer || item.stats || item.price || ''

        };

        // Use btoa + encodeURIComponent to safely embed ANY character in a data attribute

        const safeDataAttr = btoa(encodeURIComponent(JSON.stringify(itemDataObj)));
        ...
        // Encode the URL for safe HTML attribute embedding

        const safeUrlAttr = encodeURIComponent(targetUrl);



        html += `

            <div class="search-result-item" data-url="${safeUrlAttr}" data-item="${safeDataAttr}">
    ...
    // Attach click handlers via event delegation (safe from HTML injection)

    trendPanel.querySelectorAll('.search-result-item[data-url]').forEach(el => {

        el.addEventListener('click', () => {

            const url = decodeURIComponent(el.getAttribute('data-url'));

            const itemB64 = el.getAttribute('data-item');

            handleResultClick(url, decodeURIComponent(atob(itemB64)));
    ...
    const signImages = async () => {
        ...
        // Wait for getAuthorizedUrl if it's not ready yet (auth-utils.js may still be loading)
        ...
        images.forEach(async (img) => {
            ...
            // Cloudinary images are public — no signing needed
...
function renderHistoryAndTrends() {
    ...
    const historyHtml = NavbarState.search.history.map(item => {
        ...
        return `<div class="recent-item" onclick="setSearch('${safeTerm}', event, '${historyObj.type}', '${historyObj.id || ''}')">
...
window.setSearch = function (text, event, type = 'text', id = '') {
    ...
    if (event) event.stopPropagation(); // Keep dropdown open when clicking term
    ...
    if (searchInput) {
        ...
        // If it was a rich click (user/product), we can navigate directly

        if (type === 'user') {

            saveToHistory({ type: 'user', term: text, subtitle: 'Producer', id: id });

            window.location.href = `/@${encodeURIComponent(text)}`;

            return;

        } else if (type === 'beat' || type === 'kit') {

            saveToHistory({ type: type, term: text, id: id });
        ...
        saveToHistory({ type: 'text', term: text });

        searchInput.focus(); // Ensure input keeps focus to avoid closing the dropdown immediately



        // Instead of triggering input event (which debounces and just shows dropdown),

        // instantly perform the search and redirect if exact match.
...
window.handleResultClick = async function (url, itemDataString) {
    ...
    try {
        ...
        if (itemDataString) {
            ...
            } else if (item.type === 'beat' || item.type === 'kit') {

                termObj = { type: item.type, term: item.title, subtitle: item.subtitle, img: item.img, id: item.id };
